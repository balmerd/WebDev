...
// min-width add init
function initMinWidthAdd() {
	var header = jQuery('#header');
	var logo = header.find('.logo-box');
	var nav = header.find('#nav');
	var banner = header.find('a.ad');
	var m = parseInt(banner.css('marginLeft')) + parseInt(banner.css('marginRight'));
	if(banner.length) {
		jQuery('body').css({
			minWidth:logo.outerWidth(true) + nav.outerWidth(true) + banner.outerWidth(true) + m
		});
	} else {
		jQuery('body').css({
			minWidth:logo.outerWidth(true) + nav.outerWidth(true)
		});
	}
	jQuery('body').css({
		overflowX:'hidden'
...
// icons sorter init
function initIconsSorter() {
	jQuery('.map').each(function() {
		var map = jQuery(this);
		var cameraCheckbox = map.find('.cameras-checkbox');
		var incidentsCheckbox = map.find('.incidents-checkbox');
		var constructionCheckbox = map.find('.construction-checkbox');
		var inactiveConstructionCheckbox = map.find('.inactive-construction-checkbox');
		
		var cameraIcons = map.find('.cameras-link');
		var incidentsIcons = map.find('.incidents-link');
		var constructionIcons = map.find('.construction-link');
		var inactiveConstructionIcons = map.find('.inactive-construction-link');
		...
		if(incidentsCheckbox.is(':checked')) {
			incidentsIcons.show();
		} else {
			incidentsIcons.hide();
		}
		if(constructionCheckbox.is(':checked')) {
			constructionIcons.show();
		} else {
			constructionIcons.hide();
		}
		if(inactiveConstructionCheckbox.is(':checked')) {
			inactiveConstructionIcons.show();
		} else {
			inactiveConstructionIcons.hide();
		}
		
		cameraCheckbox.click(function() {
			if(cameraCheckbox.is(':checked')) {
				cameraIcons.show();
			} else {
				cameraIcons.hide();
		...
		incidentsCheckbox.click(function() {
			if(incidentsCheckbox.is(':checked')) {
				incidentsIcons.show();
			} else {
				incidentsIcons.hide();
		...
		constructionCheckbox.click(function() {
			if(constructionCheckbox.is(':checked')) {
				constructionIcons.show();
			} else {
				constructionIcons.hide();
		...
		inactiveConstructionCheckbox.click(function() {
			if(inactiveConstructionCheckbox.is(':checked')) {
				inactiveConstructionIcons.show();
			} else {
				inactiveConstructionIcons.hide();
...
function initLayout() {
	...
	var warningPanel = map.parent().find('.warrning');
	var footer = jQuery('#footer');
	var adCol = main.find('.ad-col');
	var adChildrens = adCol.children();
	var p = parseInt(main.css('paddingTop')) + parseInt(main.css('paddingBottom'));
	var fixedBlock = jQuery('.fixed-box');
	
	if(adCol.length) {
		var minH = adCol.height();
	} else {
		var minH = parseInt(jQuery('body').css('minHeight'));
	}
	
	var column = jQuery('.aside .info-column');
	var columnP = parseInt(column.css('paddingTop')) + parseInt(column.css('paddingBottom'));
	var scrollWrapper = column.find('.scrollable-area-wrapper');
	var scrollArea = scrollWrapper.find('.scrollable-area');
	
	adChildrens.eq(adChildrens.length - 1).css({paddingBottom:0});
	jQuery('body').css({
		minHeight:minH
	});
	...
	function handleResize() {
		map.css({height:''});
		mapImg.css({height:''});
		scrollWrapper.css({height:''});
		scrollArea.css({height:''});
		
		if(warningPanel.length) {
			if(win.height() > minH + header.outerHeight() + footer.outerHeight()) {
				map.css({
					height:win.height() - header.outerHeight() - footer.outerHeight() - p + 2 - warningPanel.outerHeight(true) - parseInt(warningPanel.css('marginBottom'))
				});
			} else {
				map.css({
					height:minH  - p + 5  - warningPanel.outerHeight(true) - parseInt(warningPanel.css('marginBottom'))
				});
			}
			scrollWrapper.css({
				height:map.height() + warningPanel.outerHeight(true) - parseInt(warningPanel.css('marginBottom')) - 10
			});
			scrollArea.css({
				height:map.height() + warningPanel.outerHeight(true) - parseInt(warningPanel.css('marginBottom')) - 10
			});
			...
			if(win.height() > minH + header.outerHeight() + footer.outerHeight()) {
				map.css({
					height:win.height() - header.outerHeight() - footer.outerHeight() - p - 2
				});
			} else {
				map.css({
					height:minH  - p + 1 
		...
		fixedBlock.css({
			left:win.width()  - map.offset().left - 160
		});
...
function initSlideMenu() {
	...
	var win = jQuery(window);
	var fixedBlock = jQuery('.fixed-box');
	var offsetT = parseInt(fixedBlock.css('top'));
	jQuery('.aside .info-column').each(function() {
		var holder = jQuery(this);
		var opener = holder.find('a.resize-btn');
		var slide = holder.find('.scrollable-area-wrapper');
		...
		if(holder.hasClass(activeClass)) {
			...
			slide.css({
				marginLeft:-slide.outerWidth() - 10
			});
		}
		opener.click(function(e) {
			fixedBlock.css({
				position:'fixed',
				top:fixedBlock.offset().top,
				left:'auto',
				right:30
			});
			if(holder.hasClass(activeClass)) {
				...
				slide.stop().animate({
					...
					fixedBlock.css({
						...
						right:'auto',
						top:offsetT,
						left:win.width()  - fixedBlock.parent().offset().left - 160
					});
				}});
			} else {
...
function initTooltip() {
	jQuery('a[title]').not('.incidents-list li a').hoverTooltip({
		attribute: 'title',
		positionTypeX: 'left',
...
;(function($){
	$.fn.hoverTooltip = function(o) {
		var options = $.extend({
			tooltipStructure: '<div class="hover-tooltip"><div class="tooltip-text"></div><span class="bullet"><span></div>',
		...
		function initTooltip(item) {
			...
			if(item.attr('rel')) {
				...
				var link = jQuery('[title="'+rel+'"]');
				link.hover(function() {
					tooltipTextBox.html(tooltipText);
					tooltip.appendTo(document.body).show().css({
						top:item.offset().top - tooltip.outerHeight() - 25,
						left:item.offset().left - tooltip.outerWidth(true) + 90
					});
				}, function() {
					hideTooltip();
				});
				...
				opener.bind(options.event, function(){
					if(!slider.is(':animated')) {
						if(item.hasClass(options.activeClass)) {
							...
							slider.slideDown({duration:options.animSpeed, complete:function() {
								jcf.customForms.replaceAll();
							}});
